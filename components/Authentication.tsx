...
import { googleSignUpApi } from "@/services/api";
  ...
  return (
    ...
    >
      <div className="border-2 border-gray-100 w-fit h-fit rounded-md">
        {toggle ? (
          <SignUp choose={signUpToggle} />
        ) : (
          <SignIn choose={signInToggle} />
        )}
      </div>
    </div>
  );
...
const SignUp: React.FC<SignUpProps> = ({ choose }) => {
  ...
  return (
    <div className="p-5 flex flex-col gap-3 shadow-md">
      <h1 className={classNames("font-semibold text-black text-xl")}>
        Sign Up to Draftly
      </h1>
